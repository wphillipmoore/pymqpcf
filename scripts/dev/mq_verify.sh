...
echo "=== QM1: PYMQPCF.QLOCAL ==="
curl -sS -k -u "${mq_admin_user}:${mq_admin_password}" \
  -H "Content-Type: application/json" \
  -H "ibm-mq-rest-csrf-token: local" \
  -d '{"type": "runCommandJSON", "command": "DISPLAY", "qualifier": "QLOCAL", "name": "PYMQPCF.QLOCAL"}' \
...
echo "=== QM1: PYMQPCF.SVRCONN ==="
curl -sS -k -u "${mq_admin_user}:${mq_admin_password}" \
  -H "Content-Type: application/json" \
  -H "ibm-mq-rest-csrf-token: local" \
  -d '{"type": "runCommandJSON", "command": "DISPLAY", "qualifier": "CHANNEL", "name": "PYMQPCF.SVRCONN"}' \
  "${qm1_rest_base_url}/admin/action/qmgr/QM1/mqsc"
